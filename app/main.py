...
async def load_houses_from_file():
    """Загружает дома и квартиры из data/houses.json, если база пуста."""
    houses_file = Path("data/houses.json")
    if not houses_file.exists():
        print("Файл data/houses.json не найден, пропускаем загрузку")
        return
    ...
    async with async_session() as session:
        ...
        if result.scalar_one_or_none():
            print("База уже содержит дома, загрузка из файла не требуется")
            return
        ...
        for b in data.get("buildings", []):
            ...
            await session.flush()  # получаем id дома
